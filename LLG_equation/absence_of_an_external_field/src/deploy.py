...
def characteristic_time(predicted_larmor_frequencies, larmor_frequencies,  field_values, alpha, frames, real_frames):
    ...
    plt.xlabel('$B^{-1} (T^{-1})$')
    plt.text(60, 10.8e-8, r'$\tau_B = \frac{1}{\alpha \gamma}\cdot \frac{1}{B}$', fontsize=15)
    plt.ylabel(r'$\tau_B $')
    plt.legend()
    plt.show()
    ...
    # plt.text(0, 0.3, r'$tan(\theta) = tan\theta_0 exp(\lambda w_k (t-t_0))$', fontsize=15)
    # plt.xlabel('time (s)')
...
def characteristic_time_simulation_data(larmor_frequencies, field_values, alpha,  frames):
    ...
    plt.plot(time, y,'o')
    plt.plot(time, y_fit, 'r-')
    plt.title('Simulation data: alpha ='+str(alpha)+'  B='+str(field_values[items-3])+'')
    plt.text(0, 0.3, r'$tan(\theta) = tan\theta_0 exp(\lambda w_k (t-t_0))$', fontsize=15)
    plt.xlabel('time (s)')
    plt.ylabel(r'$tan(\theta) $')
    plt.legend()
...
def plot_predictions(magnetic_field, inputs, predictions):
    ...
    simulation_inputs, targets = plot_simulation_data(0.05, 0.008)
