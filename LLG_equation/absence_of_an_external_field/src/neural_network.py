...
def main():
    ...
    ''' Prepare Data'''
    data = pd.read_csv("data_set_anisotropia.csv",sep=',')
    ...
    training_x, x_test, training_y, y_test = train_test_split(scale_x, scale_y, test_size = 0.05)
...
def train_model(model, training_x, training_y, x_test, y_test):
    ...
    results = model.fit(training_x, training_y, validation_data=(x_test,y_test), epochs=1000, batch_size=3200, callbacks=[es]) 
